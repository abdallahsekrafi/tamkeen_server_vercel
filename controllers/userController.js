...
export const getUser = async (req, res, next) => {
  try {
    ...
    // id ?? userId: if id (optional params) from params not exist we use userId from body (from auth middleware)
...
// update user (exemple: profileUrl)
export const updateUser = async (req, res, next) => {
  try {
    const { profileUrl } = req.body;

    if (!profileUrl) {
      next("Please provide the profile image");
      return;
    }
    ...
    // creating a new token if nessery
...
export const acceptRequest = async (req, res, next) => {
  try {
    ...
    if (status === "Accepted") {
      ...
      if (!user.friends.includes(newRes?.requestFrom)) {
        user.friends.push(newRes?.requestFrom);
        await user.save();
      }

      const friend = await Users.findById(newRes?.requestFrom);
      if (!friend.friends.includes(newRes?.requestTo)) {
...
// we suggest frends (option:based on disability type)
export const suggestedFriends = async (req, res) => {
  try {
    ...
    const suggestedFriends = await Users.find({ _id: { $nin: friends } })
      .limit(10)
      .select(
        "firstName lastName profileUrl isDisabled isFMDisabled isMental isMotor isHearing isVisual isPsychological -password"
      );
